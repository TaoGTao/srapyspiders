...
class AmqPoster:
    def __init__(self, que, cds, mq_info, virtual_host, exchange=''):
        credentials = pika.PlainCredentials(*cds)
        connection = pika.BlockingConnection(
            pika.ConnectionParameters(credentials=credentials, **mq_info, virtual_host=virtual_host,
                                      heartbeat=0))  # 可能因为连接失败导致异常
    ...
    def consume(self, callback, when_stop=None, requeue=True):
        ...
        self.channel.basic_consume(self.wrapper(callback, when_stop, requeue), queue=self.que)
...
# CNCT = dict(host='192.168.0.52', port=5672, virtual_host="/", )

# 示例应用
# amq = AmqPoster(QUE, CDS, CNCT)
# amq.send("test") # producer
# amq.consume(cbk) # consumer
# amq.close()